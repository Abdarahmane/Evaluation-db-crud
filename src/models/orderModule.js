...
async function updateOrder(connection) {
    ...
    customerId = readline.questionInt("New Customer ID: ");
...
async function viewOrderDetails(connection) {
    const orderId = readline.questionInt("Enter the order ID to view details: ");

    if (!(await checkOrderExists(connection, orderId))) return;

    try {
        const [orderResults] = await connection.query('SELECT * FROM orders WHERE id = ?', [orderId]);
        const [detailsResults] = await connection.query('SELECT * FROM order_details WHERE order_id = ?', [orderId]);
        ...
        if (orderResults.length === 0) {
            ...
            if (detailsResults.length === 0) {
                console.log("No details for this order.");
            } else {
                console.log("Order Details:");
                console.table(detailsResults);
            }
        }
    } catch (err) {
        displayError('Error viewing order details: ' + err.message);
    }
}
